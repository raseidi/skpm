...
def test_temporal():
    with TemporaryDirectory() as tmpdirname:
        ...
        )  # training set larger than test set
        assert (
            train.shape[1] == bpi.dataframe.shape[1]
        )  # same number of columns
        assert (
            train.shape[0] + test.shape[0] == bpi.dataframe.shape[0]
...
def test_unbiased():
    with TemporaryDirectory() as tmpdirname:
        ...
        assert (
            train.shape[0] + test.shape[0] < bpi.dataframe.shape[0]
        )  # unbiased split reduces the number of events
