...
def traces_to_ngrams(traces: list[Union[list, np.array]], N: int = 3) -> tuple[list, set]:
    """
...
class EncodedNgrams(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X, y=None):
        ...
        if self.new_ngrams_:
            warnings.warn(f"Found {len(self.new_ngrams_)} new n-grams. Call `self.new_ngrams_` to see them.")
...
dummy_log = pd.DataFrame({"caseid": [1, 1, 1, 2, 2, 2, 3, 3, 3],
