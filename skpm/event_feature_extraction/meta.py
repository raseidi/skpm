...
class _DigraphFeatures:
    @classmethod
    def _frequency_matrix(cls, traces: list, set_of_states: set) -> tuple[np.ndarray, dict, dict]:
        """
    ...
    def nodes_in_cycles(cls, frequency_matrix, max_cycle_length):
        ...
        in_cycle = [False] * num_nodes  # Initialize list to store whether each node is in a cycle
        
        for n in range(2, max_cycle_length + 1):
            matrix_power = np.linalg.matrix_power(frequency_matrix, n)
            for i in range(num_nodes):
                if matrix_power[i, i] > 0:
                    in_cycle[i] = True  # Mark node i as in a cycle if diagonal entry is non-zero
